...
module.exports = {
	...
	angencyDetails: async (Request) => {
		const angency_id = Request.params.angency_id;
		const condition = {
			conditions: {
				id: angency_id,
			},
		...
		const result = await DB.find('agencies', 'all', condition);
		if (result.length === 0) throw new ApiError('Agenzia non valida', 400);
		return {
			message: 'Lista Agenzie',
			data: {
				postcodeInfo: lat_long,
				result: app.addUrl(result, 'image')[0],
			},
		};
	},
